...
struct Serializer<Keyword>
{
    static nlohmann::json serialize(const Keyword& keyword)
    {
        return {
            ...
            {"type", keyword.type},
            {"start", keyword.valid_range.begOffset},
            {"end", keyword.valid_range.endOffset},
        };
    }
};
...
{
    server.add_command<std::string>(
        "keywords", [this](std::string xpath) { return keywords_for_path(repository.get_document(), xpath); });
