...
{
    return {message["xpath"].get<std::string>(), message["offset"].get<uint32_t>(),
            message["identifier"].get<std::string>()};
}

nlohmann::json Serializer<TextLocation>::serialize(const TextLocation& result)
{
    return {{"xpath", *result.path}, {"start", result.range.begOffset}, {"end", result.range.endOffset}};
}

nlohmann::json Serializer<TextRange>::serialize(const TextRange& range)
{
    return {{"start", range.begOffset}, {"end", range.endOffset}};
}
...
bool TextRange::contains(uint32_t offset) { return begOffset <= offset && offset <= endOffset; }
